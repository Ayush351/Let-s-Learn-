...
We need to find if we can partition the array into two subsets such that the sum of elements
 of each subset is equal to the other.
...
bool tabu(int n, vector<int>& arr) {
    int totSum = 0;
...
bool opti(int n, vector<int>& arr) {
    int totSum = 0;
