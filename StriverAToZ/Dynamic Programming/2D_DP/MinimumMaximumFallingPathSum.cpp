/*
We are given an ‘N*M’ matrix. We need to find the maximum path sum from any cell
of the first row to any cell of the last row.
...
int getMaxPathSum(vector<vector<int>>& matrix) {
    ...
    //which represents the maximum path sums ending at each cell
    int maxi = INT_MIN;
    for (int j = 0; j < m; j++) {
...
int getMaxPathSum_opti(vector<vector<int>>& matrix) {
    int n = matrix.size(); // Number of rows in the matrix
    int m = matrix[0].size(); // Number of columns in the matrix
...
int main() {
    ...
    cout << getMaxPathSum_opti(matrix);
